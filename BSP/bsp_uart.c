...
void UART_MspInit(void)
{
	...
	HAL_UART_Receive_IT(&huart4,(unsigned char *)urd4,lenmax4);
...
//		printf("\r\nCR1=%04x",huart3.Instance->CR1);
		...
		if(huart==&huart4) HAL_UART_Receive_IT(huart,(unsigned char *)urd4,lenmax4);
//		printf("\r\nuart3错误重启");
	}
...
//	SET_BIT(huart->Instance->CR3, USART_CR3_DMAR);
	...
	if((huart->Instance->CR3 & USART_CR3_DMAR) !=USART_CR3_DMAR)	//没开启
	{
		...
		i = huart3.Instance->ISR; //清除状态寄存器SR（F0的HAL库USART_TypeDef结构体中名字为ISR：USART Interrupt and status register），读取SR可以清楚该寄存器
		i = huart3.Instance->RDR; //读取数据寄存器中的数据,读取DR（
	}
