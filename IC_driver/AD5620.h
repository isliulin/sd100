...
//	flag_4g_ready=0;
	...
	if(UART_Receive_s(&huart2,&udata, &ulen,100) != 2) return 0;
//	printf("\r\n4g recv:");	
...
//		HAL_UART_Transmit_IT(&huart2, buf, strlen((char *)buf));
	...
	if(UART_Receive_s(&huart2,&udata, &ulen,1000) != 2) return -1;
	printf("\r\n4g recv:");	
  for(i=0;i<ulen;i++)	printf("%c",udata[i]);	
...
//	OS_ERR      err;
	...
	UART_data_clear(&huart2);
	HAL_UART_Transmit_IT(&huart2, (unsigned char *)"AT+CSQ\r\n", strlen("AT+CSQ\r\n"));
	
	if(UART_Receive_s(&huart2,&udata, &ulen,1000) == 2)	//收到数据
	{
