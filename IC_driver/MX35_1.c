...
{
	HAL_SPI_Transmit(&MX35_SPI, &byte_value, 1,10);
 ...
 */
ReturnMsg MX35_PAGE_READ( uint32_t flash_address )
{
	u8 sendbuf[4];
...
int CMD_READ_CACHE_PAGE( uint16_t col_address, u8 * rbuf, uint16_t r_size) 
{
    uint32_t index;
    uint32_t dat_temp =0;
    uint8_t buff[4];
	...
	buff[1]=((col_address>>8)|(0x01<<4)); //0x00-2112;		0x01-2048;		0x10-64;		0x11-16
	buff[2]=col_address;
	buff[3]=0;
	hdl_Spi_SendNbyte(buff,4);
	
	hdl_Spi_RecvNbyte(rbuf,2048);
