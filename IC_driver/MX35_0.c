...
ReturnMsg MX35_Start()
{
	if( CheckStatus( SR0_OIP ) != READY ) return Flash_F_BUSY;
}

ReturnMsg PAGE_READ(uint32_t page_address ) //13
{
	u8 buff[4];
...
{
	if(MX35_Start()==Flash_F_BUSY) return Flash_OperationTimeOut;
	if(PAGE_READ( page_address )==Flash_OperationTimeOut) return Flash_OperationTimeOut;
...
ReturnMsg CMD_BE( u32 flash_address ) //擦除flash
{
    ...
    // Check flash address
    if( flash_address > Flash_Page_Size ) return Flash_AddrInvalid;
    /* Check flash is F_BUSY or not */
    if( CheckStatus( SR0_OIP ) != READY ) return Flash_F_BUSY;
